...
class ConnectionPool extends EventEmitter {
  ...
  _connect (callback) {
    ...
    // if the pool is not connected but a pool exists then connect was
    // called whilst the pool is being closed
    if (this.pool && this.pool.destroyed) {
      debug('pool(%d): pool connect called when pool is closing, queuing connect', IDS.get(this))
      this.close(err => {
        if (err) {
    ...
    this._poolCreate().then((connection) => {
      ...
      return this._poolDestroy(connection).then(() => {
        // this feels risky because it could lead to permanently unresolved promises / callbacks
        // just because one thread has called connect and another has called close shouldn't mean
        // the promises never resolve, it feels like it should either error or reconnect
        if (!this._connecting) {
          this._connectStack = []
