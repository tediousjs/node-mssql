...
const { IDS, objectHasProperty, getPoolId, isValidParameterName } = require('../utils')
...
class PreparedStatement extends EventEmitter {
  ...
  input (name, type) {
    if (!isValidParameterName(name)) {
      throw new PreparedStatementError(`Invalid parameter name '${name}'. Parameter names may only contain identifier characters`, 'EINJECT')
    }
    ...
    this.parent.acquire(this, (err, connection, config) => {
      ...
      try {
        ...
        for (const name in this.parameters) {
          ...
          params.push(`@${name} ${declare(param.type, param)}${param.io === 2 ? ' output' : ''}`)
        }
      } catch (e) {
        this.parent.release(this._acquiredConnection)
        this._acquiredConnection = null
        this._acquiredConfig = null
        return callback(new PreparedStatementError(e, 'EREQUEST'))
      }
      req.input('params', TYPES.NVarChar, params.join(','))
