...
// A table-valued parameter's type name is emitted into the parameter declaration, so it
// is checked the way the server reads it: one or more parts separated by `.`, each either
// bare, holding only the characters an identifier is made of, or quoted in brackets or
// double quotes, where anything goes until the closing mark. Nothing that gets through
// can end the declaration and have the rest read as SQL.
const TYPE_NAME_PART = '(?:\\[(?:[^\\]]|\\]\\])*\\]|"(?:[^"]|"")*"|[\\p{L}\\p{N}\\p{M}\\p{Pc}@#$]+)'
const TYPE_NAME = new RegExp(`^${TYPE_NAME_PART}(?:\\.${TYPE_NAME_PART})*$`, 'u')
