...
const { IDS, objectHasProperty, assertSafeColumnName, assertSafeTypeName, assertSafeTypeSize, assertSafeProcedureName } = require('../utils')
  ...
  */
  ...
  _bulk (table, options, callback) {
    super._bulk(table, options, err => {
      ...
      try {
        table._makeBulk()
        // Checked before a connection is borrowed, so that a rejected name cannot leave
        // the pool short of one.
        table.columns.forEach(col => assertSafeColumnName(col.name))
        // tedious writes these into `insert bulk ... WITH (ORDER (<key> <direction>))`
        // UNQUOTED — unlike a column name, which it emits as `[name]`. So the quoted-identifier
        // rule does not apply here: what matters is that the key cannot end the clause. It is
        // checked as an identifier, bare or quoted, which is what that position accepts. The
        // checked keys are rebuilt into the options actually handed to the driver, so the value
        // that was validated is the one emitted even if the caller's object changes after.
      } catch (e) {
      ...
      this.parent.acquire(this, (err, connection) => {
        ...
        // A connection is held from here on, so anything that can throw has to hand the
        // error back through `done`, which releases it. That includes newBulkLoad itself:
        // tedious validates the bulk options there and throws for a bad `order` direction,
        // which would otherwise strand the borrowed connection.
        let bulk
        let statement
        try {
          let bulkOptions = options
          if (options && options.order) {
            bulkOptions = Object.assign({}, options, {
              order: Object.fromEntries(Object.entries(options.order)
                .map(([key, direction]) => [assertSafeTypeName(key), direction]))
            })
          }
          bulk = connection.newBulkLoad(table.path, bulkOptions, done)
          for (const col of table.columns) {
            // tedious emits this name as a quoted identifier of its own when it builds
            // the `insert bulk` statement, so it is checked here as well as in
            // Table#declare, in case the name changed since it was checked above.
            bulk.addColumn(assertSafeColumnName(col.name), getTediousType(col.type), { nullable: col.nullable, length: assertSafeTypeSize(col.length), scale: assertSafeTypeSize(col.scale), precision: assertSafeTypeSize(col.precision) })
          }

          if (table.create) {
            const objectid = table.temporary ? `tempdb..[${table.name}]` : table.path
            statement = `if object_id('${objectid.replace(/'/g, '\'\'')}') is null ${table.declare()}`
          }
        } catch (e) {
          return done(e)
        }
        ...
        if (table.create) {
          const req = new tds.Request(statement, err => {
  ...
  _query (command, callback) {
    super._query(command, err => {
      ...
      this.parent.acquire(this, (err, connection, config) => {
        ...
        // Give up before the request is sent: the listeners attached just above have to
        // come off the connection before it goes back to the pool, or they accumulate on
        // it and fire for whoever borrows it next.
        const abandon = (err) => {
          delete this._cancel
          ...
          if (Object.keys(this.parameters).length) {
            for (const name in this.parameters) {
              ...
              try {
                ...
                return abandon(new RequestError(e, 'EPARAM'))
              }
            }
            ...
            try {
              for (const name in this.parameters) {
                ...
                const param = this.parameters[name]
                // a connection is held here, so a rejected declaration has to be reported
                // through the path that releases it
                declarations.push(`@${name} ${declare(param.type, param)}`)
              }
            } catch (e) {
              return abandon(new RequestError(e, 'EREQUEST'))
            }
        ...
        try {
          if (!this._isBatch) {
            for (const name in this.parameters) {
              ...
              const param = this.parameters[name]
              // tedious builds the sp_executesql declaration from these, so a rejected size
              // has to be reported through the path that releases the connection
              if (param.io === 1) {
                req.addParameter(param.name, getTediousType(param.type), parameterCorrection(param.value), { length: assertSafeTypeSize(param.length), scale: assertSafeTypeSize(param.scale), precision: assertSafeTypeSize(param.precision) })
              } else {
                req.addOutputParameter(param.name, getTediousType(param.type), parameterCorrection(param.value), { length: assertSafeTypeSize(param.length), scale: assertSafeTypeSize(param.scale), precision: assertSafeTypeSize(param.precision) })
              }
            }
          }
        } catch (e) {
  ...
  _execute (procedure, callback) {
    super._execute(procedure, err => {
      ...
      this.parent.acquire(this, (err, connection, config) => {
        ...
        // A connection is held from here on. handleError builds a fresh Error and would
        // relabel a rejected identifier as EREQUEST, so anything that can throw here is
        // handed back through this instead, which keeps the code and releases the connection.
        const abandon = (err) => {
          delete this._cancel
        ...
        let procedureName
        try {
          procedureName = assertSafeProcedureName(procedure)
        } catch (e) {
          // wrapped like every other rejection here, so the caller sees the same error class
          // whichever driver is in use; MSSQLError keeps the EINJECT code through the wrap
          return abandon(new RequestError(e, 'EREQUEST'))
        }
        ...
        const req = new tds.Request(procedureName, err => {
          // to make sure we handle no-sql errors as well
          if (err && (!errors.length || (errors.length && err.message !== errors[errors.length - 1].message))) {
            err = new RequestError(err, 'EREQUEST')
            if (this.stream) this.emit('error', err)
        ...
        try {
          ...
          return abandon(error instanceof RequestError ? error : new RequestError(error, 'EREQUEST'))
        }
      })
    })
  }
