...
class ConnectionPool extends base.ConnectionPool {
  ...
  _poolValidate (tdsConn) {
    return new base.Promise((resolve, reject) => {
      ...
      setTimeout(() => {
        if (!didValidate) {
          debug('connection %d validation timed out after %d', connId, this.config.requestTimeout)
          reject(new Error('Validation of connection timed out after config.requestTimeout'))
        }
