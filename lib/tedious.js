...
class ConnectionPool extends base.ConnectionPool {
  ...
  /**
   * Establish whether or not we have an actual connection before loaning it out
   * @param tdsConn tedious.Connection object to SQL server that's being requested
   * @private
   */
  _poolValidate (tdsConn) {
    const validateRequestTimeout = this.config.requestTimeout
    return new base.Promise((resolve, reject) => {
      let didValidate = false
      let connId = IDS.get(tdsConn)
      const request = new tds.Request('SELECT 1', (err) => {
        didValidate = true
        if (tdsConn.closed || tdsConn.hasError) {
          debug('connection %d closed or has error', connId)
          return resolve(false)
        } else if (err) {
          debug('connection %d validate error %s', connId, err)
          return resolve(false)
        }
        debug('connection %d validated', connId)
        resolve(true)
      })
      setTimeout(() => {
        if (!didValidate) {
          debug('connection %d validation timed out after %d', connId, validateRequestTimeout)
          reject(new Error('Validation of connection timed out after config.requestTimeout: ' + validateRequestTimeout))
        }
      }, validateRequestTimeout)
      tdsConn.execSql(request)
    })
  }
