...
class ConnectionPool extends EventEmitter {
	...
	_debug(msg) {
		return __guard__(this._debugStream, x => x.write(`${String(msg).replace(/\x1B\[[0-9;]*m/g, '')}\n`));
	}
...
class PreparedStatement extends EventEmitter {
	...
	*/
	...
	_prepare(Request, statement, callback) {
		debug('ps: prepare');
		...
		this.parent.acquire(this, (err, connection) => {
			...
			req.execute('sp_prepare', err => {
				...
				this._handle = req.parameters.handle.value;
				this.prepared = true;
	...
	*/
	...
	_execute(Request, values, callback) {
		const req = this.lastRequest = new Request(this);
		...
		req.execute('sp_execute', (err, recordsets, returnValue) => {
			...
			callback(null, (this.multiple ? recordsets : recordsets[0]), req.rowsAffected);
		})
...
class Transaction extends EventEmitter {
	...
	*/
	...
	rollback(callback) {
		...
		return new promise_library((resolve, reject) => {
			return this._rollback(function(err) {
				if (err) return reject(err);
				this.emit('rollback', this._aborted);
				debug('tran: rollback ok');
				resolve();
...
class Request extends EventEmitter {
	...
	*/
	...
	input(name, type, value) {
		...
		if (__guard__(value, x => x.valueOf) && !(value instanceof Date)) { value = value.valueOf(); }
	...
	*/
	...
	batch(batch, callback) {
		...
		if (this.stream || 'function' === typeof callback) {
			this._batch(batch, (err, recordset) => {
				if (this.stream) {
					if (err) this.emit('error', err);
					return this.emit('done', this.rowsAffected);
				}
				
				callback(err, recordset, this.rowsAffected);
			})
			return this;
		}
		
		return new promise_library((resolve, reject) => {
			return this._batch(batch, (err, recordset) => {
				if (err) return reject(err);
				resolve(recordset);
			})
		})
	...
	*/
	...
	bulk(table, callback) {
		...
		if (this.stream || 'function' === typeof callback) {
			this._bulk(table, (err, recordset) => {
				if (this.stream) {
					if (err) this.emit('error', err);
					return this.emit('done', this.rowsAffected);
				}
		...
		return new promise_library((resolve, reject) => {
			return this._bulk(table, (err, recordset) => {
				if (err) return reject(err);
				resolve(recordset);
			})
		})
	...
	*/
	...
	query(command, callback) {
		...
		if (this.stream || 'function' === typeof callback) {
			this._query(command, (err, recordset) => {
				if (this.stream) {
					if (err) this.emit('error', err);
					return this.emit('done', this.rowsAffected);
				}
		...
		return new promise_library((resolve, reject) => {
			return this._query(command, function(err, recordset) {
				if (err) return reject(err);
				resolve(recordset);
			})
		})
	...
	*/
	...
	execute(command, callback) {
		...
		if (this.stream || 'function' === typeof callback) {
			this._execute(command, (err, recordset, returnValue) => {
				if (this.stream) {
					if (err) this.emit('error', err);
					return this.emit('done', returnValue, this.rowsAffected);
				}
				
				callback(err, recordset, returnValue, this.rowsAffected);
			})
			return this;
		}
		
		return new promise_library((resolve, reject) => {
			return this._execute(command, function(err, recordset) {
				if (err) return reject(err);
				resolve(recordset);
			})
		})
...
module.exports.exports.connect = function connect(Connection, config, callback) {
...
module.exports.exports.close = function close(callback) {
	return __guard__(global_connection, x => x.close(callback));
}
...
module.exports.exports.on = function on(event, handler) {
	return __guard__(global_connection, x => x.on(event, handler));
}
...
module.exports.exports.query = function query(Request, strings, ...values) {
...
module.exports.exports.batch = function batch(Request, strings, ...values) {
